...
const ScratchPadTab = ({ scratchpad }) => {

    const [isSaving, setIsSaving] = useState(false);
    const [localScratchPad, setLocalScratchPad] = useState(scratchpad);
    const debouncedScratchPad = useDebounce(localScratchPad, 500);
    ...
    const handleSave = async () => {
        try {
            await fetch('/api/questions', { method: 'PUT', body: JSON.stringify({ uid: id, scratchpad: debouncedScratchPad }) })
        }
        catch (error) {
            console.log(error)
        }
    ...
    return <div className={'max-h-[550px] product__test_scrollbar overflow-x-hidden bg-[#001528] relative'}>
        ...
        />
        { isSaving ? (
            <div role="status" className={'absolute top-0 right-[15px] w-[20px] h-[20px]'}>
