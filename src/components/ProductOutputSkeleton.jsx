...
const ProductOutputSkeleton = () => {
    ...
    const handleCodeRun = () => {
        animate('section', { display: 'flex', opacity: 1, transitionDuration: 1 })
        setTimeout(() => {
            animate('section', { display: 'none', opacity: 0, transitionDuration: 1 })
        }, [2000])
    }
    ...
    return <motion.div
        ...
        className={"bg-[#15314b] flex flex-col rounded-[4px] border-[2px] border-[#15314b] max-w-[291px] h-full w-full"}>
        <div className={"max-h-[30px] h-full flex justify-between items-center"}>
            ...
            <motion.div layout ref={scope} className={'max-h-[180px] bg-[#02203c] h-full product__test_scrollbar'}>
                <LayoutGroup>
                    <motion.section
                        layout
                        className={'bg-[#02203c] flex justify-center h-[150px] flex-1 items-center'}
                        initial={{ opacity: 1 }}
