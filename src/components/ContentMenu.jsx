...
const ContentMenuItem = ({ name, onMouseOver, selected, bgColor }) => {
   ...
   return (
       <div onMouseOver={onMouseOver}
            ...
            className={'flex justify-between cursor-pointer p-[15px] w-full font-bold text-[16px] bg-white text-[#02203c] relative rounded-tl-[4px] rounded-bl-[4px] hover:bg-[#626ee3] hover:text-white header__content_menu--hover'}>
           <span>{ name }</span>
...
const AlgoExpertMenu = () => {
    return (
        <section className={'flex flex-col p-[15px] flex-1 bg-[#626ee3] h-full text-white gap-[15px] rounded-[4px]'}>
            <ContentMenuLink href={'/'} name={'Coding Interview Questions'} />
            <ContentMenuLink href={'/'} name={'Data Structures Crash Course'} />
